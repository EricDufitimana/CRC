...
interface ReferOpportunityDialogProps {
  open: boolean;
...
export function ReferOpportunityDialog({ open, onOpenChange, opportunity, currentAdminId }: ReferOpportunityDialogProps) {
  const trpc = useTRPC();
  const queryClient = useQueryClient();
  const [selectedAdminId, setSelectedAdminId] = useState<string | null>(null);

  const { data: admins = [], isFetching } = useQuery({
    ...trpc.opportunityRequestsManagement.getAdmins.queryOptions(),
    enabled: open,
  });

  const mutation = useMutation({
    ...trpc.opportunityRequestsManagement.deferTo.mutationOptions(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [["opportunityRequestsManagement", "getOpportunityRequests"]] });
      queryClient.invalidateQueries({ queryKey: [["opportunityRequestsManagement", "getReferrals"]] });
      showToastSuccess({
        headerText: "Opportunity Referred",
        paragraphText: "The opportunity has been successfully referred.",
        direction: "right"
      });
      onOpenChange(false);
      ...
      <DialogContent className="max-w-md rounded-3xl p-6 border-gray-100 shadow-none">
            ...
            {isFetching ? (
              ...
              filteredAdmins.map((admin) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-2xl border transition-all duration-200 ${selectedAdminId === admin.id
        ...
        <DialogFooter className="pt-6 flex gap-3">
          ...
          <Button
            ...
            className="bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold shadow-[inset_-2px_2px_0_rgba(255,255,255,0.1),0_1px_6px_rgba(0,0,0,0.2)] flex-[1.5] h-11 transition-all active:scale-95"
          >
            {mutation.isPending ? (
              <>
