...
export function AttendanceOverviewSection({ records, loading }: AttendanceOverviewSectionProps) {
  ...
  const getAttendanceIcon = (percentage: number) => {
    if (percentage >= 90) return <CheckCircle weight="fill" className="h-4 w-4 text-green-600" />;
    if (percentage >= 75) return <WarningCircle weight="fill" className="h-4 w-4 text-yellow-600" />;
    return <XCircle weight="fill" className="h-4 w-4 text-red-600" />;
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-orange-100 rounded-lg">
              <Users weight="fill" className="h-5 w-5 text-orange-600" />
            </div>
            <div>
          ...
          {loading ? (
                  ...
                  <div key={index} className="group relative p-4 border border-gray-200 rounded-xl hover:border-orange-200 hover:bg-gradient-to-r hover:from-orange-50 hover:to-transparent transition-all duration-200 cursor-pointer">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-2 text-xs text-gray-500">
                          <CalendarIcon weight="regular" className="h-3 w-3" />
                    ...
                    <div className="space-y-3">
                      ...
                      <Progress
                        ...
                        className={`h-[4px] transition-all duration-300 ${attendancePercentage >= 90 ? '[&>div]:bg-statColors-1' :
                      ...
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className={`font-semibold ${attendancePercentage >= 90 ? 'text-green-600' :
            ...
            <div className="flex flex-col items-center justify-center py-16 px-4">
              <div className="mb-4 p-4 bg-orange-50 rounded-full">
                <UserGroupIcon weight="regular" className="h-8 w-8 text-orange-400" />
              </div>
