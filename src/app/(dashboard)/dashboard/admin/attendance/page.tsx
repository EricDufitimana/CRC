...
export default function AttendancePage() {
  ...
  // Calculate metrics when attendance records change
  useEffect(() => {
    calculateMetrics();
  }, [attendanceRecords]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, selectedClass, selectedDate, selectedStatus]);
  ...
  const handleRecordAttendance = async () => {
    if (!selectedWorkshop || !selectedClass) {
      alert('Please select both workshop and class');
      return;
    }
    ...
    try {
      ...
      alert('Failed to record attendance: ' + (error as Error).message);
    } finally {
  ...
  // Calculate real metrics from attendance records
  const calculateMetrics = () => {
    const today = new Date();
    const todayStr = format(today, "yyyy-MM-dd");
    
    // Calculate the previous Monday (start of last week)
    const getPreviousMonday = (date: Date) => {
      ...
      previousMonday.setDate(date.getDate() - daysToSubtract - 7); // Go back to previous Monday
    ...
    console.log('🔍 DEBUG - Date calculations:', {
      today: todayStr,
      previousMonday: previousMondayStr,
      todayDayOfWeek: today.getDay(),
      previousMondayDayOfWeek: previousMonday.getDay()
    });
    
    const todayRecords = attendanceRecords.filter(record => 
      record.created_at.startsWith(todayStr)
    );
    
    const lastWeekRecords = attendanceRecords.filter(record => 
      record.created_at.startsWith(previousMondayStr)
    );
    ...
    const totalToday = presentCount + lateCount + absentCount;

    const presentLastWeek = lastWeekRecords.filter(record => record.status === 'present').length;
    const lateLastWeek = lastWeekRecords.filter(record => record.status === 'late').length;
    const absentLastWeek = lastWeekRecords.filter(record => record.status === 'absent').length;

    // Calculate average check-in time (simplified - could be enhanced)
    const presentRecords = todayRecords.filter(record => record.status === 'present');
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 font-cal-sans">Attendance Dashboard</h1>
                            ...
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <p className="font-medium text-gray-900">{student.first_name} {student.last_name}</p>
                                          <p className="text-sm text-gray-500">{student.student_id} • {student.grade.replace("_"," ")} • {student.major_short}</p>
                              </div>
                            </div>
                            <div className="flex items-center space-x-2">
                                                             <Select 
                                 value={selectedStudents[student.id] || 'none'} 
                                 onValueChange={(value: 'present' | 'absent' | 'late' | 'excused' | 'none') => {
                                   if (value === 'none') {
              ...
              {(!selectedWorkshop || selectedClass === "all" || loadingWorkshops) && (
                <div className="flex-1 flex items-center justify-center text-gray-500">
                  <div className="text-center">
                    ...
                    <p>
                      {loadingWorkshops 
                        ? "Loading workshops for selected class..." 
                        : "Please select a class first, then choose a workshop to view students"
                      }
                    </p>
                  </div>
                </div>
            ...
            <DialogFooter className=" pt-4">
              <div className="flex items-center justify-between w-full">
                <div className="text-sm text-gray-600">
                  {selectedWorkshop && selectedClass !== "all" && !loadingStudents && (
                    <span>
