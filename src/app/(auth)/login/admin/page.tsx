  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/unauthorized`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          }
      ...
      if (error) {
        ...
        // Keep loading state true for a moment so user sees the spinner
        // The redirect will happen automatically, so we don't need to set loading to false
        setTimeout(() => {
          // Only set loading to false if we're still on this page (no redirect happened)
